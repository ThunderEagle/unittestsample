...
namespace PartialMockExample.NSubstitue.Tests {
	...
	public class FooTests {
		...
		[Test]
		public void PartialMock_NotRealCode() {
			var foo = Substitute.ForPartsOf<Foo>();

			foo.When(f => f.DoSomething()).DoNotCallBase();

			foo.When(f => f.DoSomething()).DoNotCallBase();
			foo.When(f => f.DoSomething()).Do(x => { Console.WriteLine("{0}-MOCK CODE", MethodBase.GetCurrentMethod().Name); });
